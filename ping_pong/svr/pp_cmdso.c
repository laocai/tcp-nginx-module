    ...
    int slot, ngx_tcp_cycle_ctx_t *cycle_ctx)
{
    ...
    if (0 != (*add_h)(cycle_param, PP_CMD_CS, PP_CMD_CS, pp_pkg_handler)) {
        ngx_tcp_log_error(&cycle_ctx->export_func.log, NGX_TCP_LOG_ERR, 0, 
    ...
    ngx_tcp_log_error(&cycle_ctx->export_func.log, NGX_TCP_LOG_INFO, 0, 
        "cmdso_load|slot=%d|socketfd_shm_info=%p", 
            slot, g_cycle_ctx->export_data.socketfd_shm_info);
...
pp_pkg_handler(ngx_tcp_ctx_t *ctx, const u_char *pkg, int pkg_len)
{
    ...
    *((int *) ctx->cmdso_sessioin[pp_cmdso_slot]) += 1;
    ...
    if (g_cycle_ctx->export_func.conf_get_str(ini_section, pp_key, &pp_val) != 0) {
    ...
    (*(g_cycle_ctx->export_func.send_data))(ctx, pkg, pkg_len);
